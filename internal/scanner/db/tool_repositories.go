...
func NewLiveCrawledUrlsRepository(db *DB) *StringRepository {
	return NewStringRepository(db,
		"live_crawled_urls",
		"url",
		`CREATE TABLE IF NOT EXISTS live_crawled_urls (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            domain_id INTEGER NOT NULL,
            url TEXT NOT NULL,
            FOREIGN KEY (domain_id) REFERENCES domains(id) ON DELETE CASCADE
        )`,
		`INSERT INTO live_crawled_urls (domain_id, url) VALUES (?, ?)`,
	)
}
...
type ToolsRepository struct {
	...
	LiveCrawledUrls *StringRepository
}
...
func NewToolsRepository(db *DB) *ToolsRepository {
	return &ToolsRepository{
		...
		LiveCrawledUrls: NewLiveCrawledUrlsRepository(db),
	}
}
